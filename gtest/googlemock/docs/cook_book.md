...
*   Having to mix a mock and a fake is often a sign of something gone wrong.
    ...
    intermediate angle when you are refactoring your code.
...
*   Obviously, you can do angle #1 without locking.
*   When doing angle #2 and #5, make sure no other thread is accessing `foo`.
    Obvious too, huh?
...
second angle is not strictly needed but it makes the syntax of using the matcher
nicer.
...
mock function's argument list. The first angle is to define an implementation
class:
